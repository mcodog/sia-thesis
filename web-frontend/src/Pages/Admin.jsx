...
const Admin = () => {
  ...
  useEffect(() => {
    const fetchSleepData = async () => {
      try {
        const response = await axiosInstance.get("/sleep-tracker/");
  ...
  useEffect(() => {
    const fetchBreathingData = async () => {
      try {
        const response = await axiosInstance.get("/api/breathing-sessions/");
        
        // Grouping sessions per day
        const sessionCounts = response.data.reduce((acc, session) => {
          const date = session.date; // Siguraduhin na may 'date' field sa response
          acc[date] = (acc[date] || 0) + 1;
          return acc;
        }, {});

        // Convert sa format na magagamit sa BarChart
        const formattedData = Object.entries(sessionCounts).map(([date, count]) => ({
          date,
          count,
        }));
        ...
        console.error("Error fetching breathing session data:", error);
      }
    };
