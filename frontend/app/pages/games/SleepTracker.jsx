...
const SleepTracker = ({ onUpdateReport }) => {
    ...
    const saveSleepData = async () => {
        try {
            ...
            console.error("Error saving sleep history:", error.response?.data || error);
        }
    };
    ...
    const loadSleepData = async () => {
        try {
            const response = await axiosInstanceWithBearer.get('/sleep-tracker/');
            const fetchedData = response.data;
            console.log("Fetched Sleep History:", fetchedData);
            fetchedData.sort((a, b) => new Date(b.date) - new Date(a.date));
            setSleepData(fetchedData);
            await AsyncStorage.setItem('sleepData', JSON.stringify(fetchedData));
        } catch (error) {
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <View style={styles.pickerContainer}>
                <Picker selectedValue={sleepHour} onValueChange={setSleepHour} style={styles.picker}>
                    {generateHourOptions().map(hour => <Picker.Item key={hour} label={hour} value={hour} />)}
                </Picker>
                <Picker selectedValue={sleepMinute} onValueChange={setSleepMinute} style={styles.picker}>
                    {generateMinuteOptions().map(minute => <Picker.Item key={minute} label={minute} value={minute} />)}
                </Picker>
                <Picker selectedValue={sleepPeriod} onValueChange={setSleepPeriod} style={styles.picker}>
            ...
            <View style={styles.pickerContainer}>
                <Picker selectedValue={wakeHour} onValueChange={setWakeHour} style={styles.picker}>
                    {generateHourOptions().map(hour => <Picker.Item key={hour} label={hour} value={hour} />)}
                </Picker>
                <Picker selectedValue={wakeMinute} onValueChange={setWakeMinute} style={styles.picker}>
                    {generateMinuteOptions().map(minute => <Picker.Item key={minute} label={minute} value={minute} />)}
                </Picker>
                <Picker selectedValue={wakePeriod} onValueChange={setWakePeriod} style={styles.picker}>
                    <Picker.Item label="AM" value="AM" />
                    <Picker.Item label="PM" value="PM" />
                </Picker>
            </View>
