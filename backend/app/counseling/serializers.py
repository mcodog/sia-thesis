...
from .models import ChatLog, Message, CounselingAnalysis, UserAnalysis, BreathingSession, SleepEntry, MoodEntry, Diary, UserProfile, CallRecord, CallRecordWithArrays
...
class CallRecordSerializer(serializers.ModelSerializer):
    class Meta:
        model = CallRecord
        fields = [
            'id', 'user', 'date', 'time', 'duration', 'recording_url', 
            'summary', 'sentiment', 'mood', 'key_phrases', 
            'risk_level', 'follow_up_needed'
        ]
        
    def to_representation(self, instance):
        representation = super().to_representation(instance)
        # Format the representation to match your desired JSON structure
        analysis = {
            'sentiment': representation.pop('sentiment'),
            'mood': representation.pop('mood'),
            'keyPhrases': representation.pop('key_phrases'),
            'riskLevel': representation.pop('risk_level'),
            'followUpNeeded': representation.pop('follow_up_needed')
        }
        representation['analysis'] = analysis
        return representation
    
    def to_internal_value(self, data):
        # Handle nested analysis field when deserializing
        if 'analysis' in data:
            analysis = data.pop('analysis')
            data['sentiment'] = analysis.get('sentiment')
            data['mood'] = analysis.get('mood')
            data['key_phrases'] = analysis.get('keyPhrases')
            data['risk_level'] = analysis.get('riskLevel')
            data['follow_up_needed'] = analysis.get('followUpNeeded')
        return super().to_internal_value(data)


class CallRecordWithArraysSerializer(serializers.ModelSerializer):
    class Meta:
        model = CallRecordWithArrays
        fields = [
            'id', 'user', 'date', 'time', 'duration', 'recording_url', 
            'summary', 'sentiment', 'mood', 'key_phrases', 
            'risk_level', 'follow_up_needed'
...
class ChatLogSerializer(serializers.HyperlinkedModelSerializer):
	...
	class Meta:
		model = ChatLog
		fields = ('id','user', 'date_created', 'last_updated', 'summary', 'analysis')
